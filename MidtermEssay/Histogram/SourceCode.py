...
def normalize_cdf(cdf, image):
    max_cdf = cdf.max() # Phương thức cdf.max() trả về giá trị lớn nhất của mảng cdf
    min_cdf = cdf.min() # Phương thức cdf.min() trả về giá trị nhỏ nhất của mảng cdf
    normalized_cdf = (cdf * 255) / (image.shape[0] * image.shape[1])  
...
def equalize_image(image):
    ...
    normalized_cdf = normalize_cdf(CDF, gray_image)  # Chuẩn hóa CDF
